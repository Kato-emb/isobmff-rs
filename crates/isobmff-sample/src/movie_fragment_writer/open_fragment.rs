...
///
/// `data_offset` is where the run starts in that media data. The two versions
/// of a `trun` write composition time offsets unsigned and signed, so one run
/// reaches either past [`i32::MAX`] or below zero — not both — and a sample is
/// added to the run before it only where that holds; see [`takes`](Self::takes).
#[derive(Clone, Debug)]
struct OpenRun {
    data_offset: u64,
...
fn build_track_fragment(
    ...
    // Why not unwrap: a `traf` refuses only runs under a `duration-is-empty`
    // header, which is never the one built here, so this stands for a `None`
    // the call does not reach.
...
fn build_track_run(
    ...
    // Why not unwrap: the rows carry the same fields by construction, the
    // first sample's flags are only given where the rows carry none, and a run
    // holding offsets no one version writes was cut before it, so this stands
    // for a `None` the call does not reach.
