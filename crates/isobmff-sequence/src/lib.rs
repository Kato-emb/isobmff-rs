...
//! and reports it as [`BoxEvent`]s, each owning the bytes it carries. It reaches
//! for no source and no destination: input is handed to it and events are taken
...
//! and the events the input completed are held until the caller takes them.
