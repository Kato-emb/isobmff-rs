...
pub trait BoxEncode {
    ...
    /// the mirror of [`BoxDecode::decode`](crate::BoxDecode::decode), which
    /// reads the same box back.
    ///
    ...
    /// * What [`encode_payload`](Self::encode_payload) reports, for the
    ///   payload written after the header.
    ///
    /// # Examples
    ///
    /// ```
    /// use isobmff_core::{BoxDefinition, BoxEncode, BoxType, Error, FieldWriter};
    ///
    /// // A box whose payload is one 32-bit sequence number
    /// struct SequenceNumberBox {
    ///     sequence_number: u32,
