...
// segment, so a nested one looks for `tests/tests/helpers/vendor.rs`.
#[path = "helpers/vendor.rs"]
mod vendor;

#[cfg(test)]
mod tests {
    use super::vendor::{ExpiryBox, OpaqueDataBox, SequenceNumberBox, VendorMarkerBox};
    use isobmff_core::{BoxDecode, BoxEncode};

    /// Writes `value` into a buffer of exactly the payload length it declares
    fn encoded(value: &impl BoxEncode) -> Vec<u8> {
        let mut buffer = vec![0; usize::try_from(value.payload_len()).unwrap()];
        value.encode_payload(&mut buffer).unwrap();
    ...
    fn an_empty_payload_reads_back_as_the_value_that_wrote_it() {
        let payload = encoded(&VendorMarkerBox);
