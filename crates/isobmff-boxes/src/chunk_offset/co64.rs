//! [`ChunkLargeOffsetBox`] (`co64`), ISO/IEC 14496-12 §8.7.5
...
/// [`ChunkOffsetEntry`](super::ChunkOffsetEntry)'s does, and is held at the
/// width the box writes it.
#[non_exhaustive]
...
/// chunk, in chunk order, as a [`ChunkOffsetBox`](super::ChunkOffsetBox) holds
/// them, at a width that reaches any offset a file can have. The spec lets a
/// `co64` state offsets a `stco` would have reached, and such a box writes back
/// as a `co64`.
///
...
mod tests {
    ...
    use super::{ChunkLargeOffsetBox, ChunkLargeOffsetEntry};

    /// Writes the payload of the box and returns the bytes it occupies
    fn encoded_payload(chunk_large_offset: &ChunkLargeOffsetBox) -> Vec<u8> {
        let mut buffer = vec![0; usize::try_from(chunk_large_offset.payload_len()).unwrap()];
        chunk_large_offset.encode_payload(&mut buffer).unwrap();
