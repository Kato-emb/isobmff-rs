...
/// while another was open — or a failure of a layer beneath, which this type
/// carries through whole rather than translating:
...
mod tests {
    ...
    fn a_kind_falls_in_the_category_its_situation_asks_for() {
        ...
        assert_eq!(
            StructureError::duplicate_box(MOOV).category(),
            Category::Malformed
        );
        assert_eq!(
            StructureError::box_out_of_order(MOOV).category(),
            Category::Malformed
        );
        assert_eq!(
        ...
        assert_eq!(
            StructureError::box_still_open(MOOV).category(),
            Category::Usage
        );
        assert_eq!(
