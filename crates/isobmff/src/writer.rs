...
#[derive(Clone, Copy, Debug)]
enum State {
    /// Taking samples for the fragments it lays out
    Writing,
...
///   ([`default-base-is-moof`](TrackFragmentHeaderBox::DEFAULT_BASE_IS_MOOF),
...
impl SampleWriter {
    ...
    fn place(&mut self, sample: Sample) -> Result<(), SampleError> {
        ...
        let data_offset = open.media_data.len() as u64;
        let carries_on = open.last_track_id == Some(track_id);
        ...
        match open
            ...
            .get(&track_id)
            .copied()
            .and_then(|position| open.tracks.get_mut(position))
        {
            Some(track) => {
            ...
            None => {
                if let Some(reached) = self.reached.get(&track_id).copied() {
                ...
                open.placed_tracks.insert(track_id, open.tracks.len());
                open.tracks.push(track);
            }
        }

        open.media_data.extend_from_slice(sample.data());
        open.last_track_id = Some(track_id);
    ...
    fn close(&mut self) -> Result<(), SampleError> {
        ...
        let measured = build_movie_fragment(open.sequence_number, &open.tracks, None)?;
        let media_data = MediaDataBox::new(open.media_data);
        ...
        let movie_fragment = build_movie_fragment(open.sequence_number, &open.tracks, Some(base))?;

        for track in &open.tracks {
            self.reached.insert(track.track_id, track.reached);
        }
        self.fragments.push_back((movie_fragment, media_data));
...
/// of the `moof` — the anchor `default-base-is-moof` establishes (§8.8.7.1).
...
mod tests {
    ...
    use isobmff_boxes::TrackFragmentBox;
    use isobmff_core::BoxEncode as _;

    use super::{
        MediaDataBox, MovieFragmentBox, MovieFragmentHeaderBox, Sample, SampleError, SampleWriter,
